...
const REVEAL_SESSION_KEY = 'oracle-reveal-shown';
...
export default function App({ initialPath }: AppProps) {
  ...
  const desktopPlayoffRef = useRef<HTMLDivElement | null>(null);
  const mobilePlayoffRef = useRef<HTMLDivElement | null>(null);
  ...
  const advancementRows = useMemo(() => {
    const visibleRows = allAdvancementRows.filter((row) => row.pMakesPlayoffs >= 0.01);
  ...
  const lockedRecord = useMemo(() => {
    ...
    for (const game of NBA_SCHEDULE) {
      if (game.isCompleted || !lockedPicks.has(game.gameId)) {
        continue;
      }
  ...
  const scrollToPlayoffSection = useCallback(() => {
    const target = isMobile ? mobilePlayoffRef.current : desktopPlayoffRef.current;
  ...
  useEffect(() => {
    if (!allGamesPicked || !revealPendingRef.current || isBusy || !projectedChampion) {
      return;
    }

    if (window.sessionStorage.getItem(REVEAL_SESSION_KEY) === '1') {
      revealPendingRef.current = false;
      return;
    }

    revealPendingRef.current = false;
    window.sessionStorage.setItem(REVEAL_SESSION_KEY, '1');
  ...
  useEffect(() => {
    if (activeTab !== 'playoffs' || !allGamesPicked) {
      return;
    }

    const timer = window.setTimeout(() => {
      scrollToPlayoffSection();
    }, 60);

    return () => {
      window.clearTimeout(timer);
    };
  }, [activeTab, allGamesPicked, mobileTab, scrollToPlayoffSection]);
  ...
  const handleNavigate = useCallback(
    (tab: 'schedule' | 'playoffs') => {
      ...
      window.history.pushState({}, '', '/playoffs');
    ...
    [allGamesPicked, isMobile, scrollToPlayoffSection],
  );
  ...
  return (
    <>
      ...
      {showCompletionReveal && projectedChampion ? (
        <div className="oracle-reveal" role="presentation">
          <div className="oracle-reveal-backdrop" onClick={() => setShowCompletionReveal(false)} />
          <div
          ...
          >
            ...
            <img className="reveal-logo" src={projectedChampion.team.logoUrl} alt={projectedChampion.team.name} />
            <h2 className="reveal-team">{projectedChampion.team.name}</h2>
            <p className="reveal-prob">Championship Probability: {(projectedChampion.probability * 100).toFixed(1)}%</p>
            <p className="reveal-record">
              Projected Record: {projectedChampion.standingRow.wins}-{projectedChampion.standingRow.losses} · #
              {projectedChampion.standingRow.projectedSeed ?? projectedChampion.standingRow.seedLabel} Seed{' '}
              {projectedChampion.team.conference}
            </p>
            <button type="button" className="reveal-cta" onClick={() => handleNavigate('playoffs')}>
              View Playoff Bracket →
            </button>
            <button type="button" className="reveal-close" onClick={() => setShowCompletionReveal(false)}>
              Continue Editing
            </button>
          </div>
        </div>
